...
export default class WXManager extends BaseSdk{
    ...
    startListener() {
        ...
        if (!this.listenseIsOpen) {
            ...
            wx.onShow(function(res){
                console.log("wx.onShow, time:", MKUtils.getCurOsMillisecond(), ", res:", res)
                // MKUtils.errorTips("onshow --- " + MKUtils.getCurOsMillisecond())
                WXManager.getInstance().checkShareResult()
            }.bind(this))

            wx.onHide(function(){
                // MKUtils.errorTips("onhide --- " + MKUtils.getCurOsMillisecond())
                console.log("wx.onHide, time:", MKUtils.getCurOsMillisecond())
                WXManager.getInstance().hideTime = MKUtils.getCurOsMillisecond()
            }.bind(this))
        }
    }
    ...
    login() {
        ...
        wx.login({
            success(res) {
                ...
                if (res.code) {
                    WXManager.getInstance().showShareMenu()
                    WXManager.getInstance().loginCode = res.code
                    WXManager.getInstance().checkUserInfo()
                } else {
    ...
    getLoginData() {
        ...
        let url = Sina_Config.WX_XYX_LOGIN_URL + "?code=" + WXManager.getInstance().loginCode + "&gamdid=" + Sina_Config.SmallGameId
        xhr.open("POST", url)
        ...
        xhr.onreadystatechange = function () {
            ...
            if (xhr.readyState == 4) {
                if (xhr.status >= 200 && xhr.status < 300) {
                    ...
                    WXManager.getInstance().initRewardVideoAd()
                }
            }
        }.bind(this);
        xhr.send()
        ...
        wx.getSetting({
            success(res) {
                if (res.authSetting['scope.userInfo'] == true) { 
                    console.log("用户已经授权")
                    WXManager.getInstance().getUserInfo()
                } else {
                    WXManager.getInstance().getLoginData()
                }
            }
        })
    }
    ...
    authSettingUserInfo() {
        ...
        if (!WXManager.getInstance().wxSDKVersionIsBig("2.0.1")) {
                ...
                if (res.authSetting['scope.userInfo'] == true) { 
                    ...
                    wx.getSystemInfo({
                        success(res) {
                            ...
                            button.onTap((res) => {
                                console.log("wx createUserInfoButton", res)
                                if (res.userInfo) {
                                    console.log("用户授权成功");
                                    WXManager.getInstance().avatarUrl = res.userInfo.avatarUrl
                                    WXManager.getInstance().nickName = res.userInfo.nickName
                                    SingleGameLogic.getInstance().wxUserInfo(WXManager.getInstance().nickName, WXManager.getInstance().avatarUrl)
                                }else {
                                    console.log("用户拒绝授权");
                                }
                                button.destroy()
    ...
    showShareMenu() {
        ...
        wx.updateShareMenu({
            withShareTicket: true,
            success:function(res) {cc.log("wx updateShareMenu success, res:", res)},
            fail:function(res) {cc.log("wx updateShareMenu failed, res:", res)}
        })
    ...
    createBannerAd(adIndex, topScale?:number) {
        ...
        if (!WXManager.getInstance().wxSDKVersionIsBig("2.0.4")) {
        ...
        let adUnitId = WXManager.getInstance().getBannerAdId(adIndex)
        if (!adUnitId) {
        ...
        if (!this.bannerAd[adIndex]) {
            ...
            topScale = topScale || WXManager.getInstance().bannerAdHeightScale
            ...
            WXManager.getInstance().bannerAd[adIndex] = wx.createBannerAd({
                adUnitId: adUnitId,
                style: {
                    left: adLeft,
                    top: adTop,
            ...
            WXManager.getInstance().bannerAd[adIndex].onError(function(res){
            ...
            WXManager.getInstance().bannerAd[adIndex].show()
        } else {
    ...
    initRewardVideoAd() {
        ...
        let adUnitId = WXManager.getInstance().getRewardVideoAdId(0)
        if (!adUnitId) {
        ...
        WXManager.getInstance().rewardVideoAd = videoAd
    ...
    playRewardVideoAd(adIndex?:number, successCallback?:any, failCallback?:any) {
        ...
        let adUnitId = WXManager.getInstance().getRewardVideoAdId(adIndex)
        if (!adUnitId) {
        ...
        WXManager.getInstance().videoAdSuccessCallback = successCallback
        WXManager.getInstance().videoAdFailCallback = failCallback
        ...
        videoAd.onError(function(res){
            ...
            videoAd.offError()
            videoAd.offClose()
            if (!WXManager.getInstance().rewardVideoAd) {return}
            MKUtils.errorTips("今日视频广告次数已用完")
            SingleGameLogic.getInstance().rewardVideoAdEnable = false
            if (WXManager.getInstance().videoAdFailCallback) {
                WXManager.getInstance().videoAdFailCallback()
            }
            SingleGameLogic.getInstance().refreshRewardType()

            WXManager.getInstance().videoAdFailCallback = null
            WXManager.getInstance().videoAdSuccessCallback = null
            WXManager.getInstance().rewardVideoAd = null
        }.bind(this))
        ...
        videoAd.onClose(res => {
            ...
            if (res && res.isEnded || res === undefined) {
                // 正常播放结束，可以下发游戏奖励
                if (WXManager.getInstance().videoAdSuccessCallback) {
                    WXManager.getInstance().videoAdSuccessCallback()
                }
            } else {
                // 播放中途退出，不下发游戏奖励
    ...
    wxSDKVersionIsBig(version) {
        ...
        return WXManager.getInstance().compareVersion(curVersion, version) >= 0
    }

    //小游戏跳转
    toMiniProgram(appIndex) {
        if (!MKUtils.isWXGame()) {return} 
        if (!WXManager.getInstance().wxSDKVersionIsBig("2.2.0")) {
            MKUtils.errorTips("当前微信版本过低，请升级")
            return
        }
        console.log("wx navigateToMiniProgram appId:", appIndex)
        let appId = WXManager.getInstance().miniProgressIdConfig[appIndex]
        if (!appId) {
